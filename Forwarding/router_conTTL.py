...
def parse_packet(IP_packet: bytes):
    ...
    parts = text.split(SEPAREITOR, 3)   # es decir, tenemos 3 splits para 4 elementos
    if len(parts) != 4:
...
def create_packet(parsed_IP_packet: dict) -> str:
    """
    Recibe un diccionario con keys dest_ip, dest_port, ttl, data
    y devuelve el string del paquete en formato: "IP;PUERTO;TTL;msg"
    """
    return (
        f"{parsed_IP_packet['dest_ip']}{SEPAREITOR}"
        f"{parsed_IP_packet['dest_port']}{SEPAREITOR}"
        f"{parsed_IP_packet['ttl']}{SEPAREITOR}"
        f"{parsed_IP_packet['data']}"
    )
...
def main():
    if len(sys.argv) != 4:
        print("No se usa así pajarón kl: python3 router.py <IP_router> <puerto_router> <rutas_textfile>")
        sys.exit(1)
    ...
    while True:
        ...
        sock.sendto(new_ip_packet, next_hop)
